...
type State = {
  ...
  // Tracing
  tracing: boolean;                   // whether we’re recording a trace right now
  traceInstrs: bril.Operation[];      // captured operations
  traceLimit: number;                 // how many instructions to record
  traceCount: number;                 // how many we’ve recorded so far
};
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  // --- trace capture ---
  if (state.tracing && state.traceCount < state.traceLimit) {
    // record the raw Bril operation
    state.traceInstrs.push(instr as bril.Operation);
    state.traceCount++;
    // once we hit the limit, stop tracing
    if (state.traceCount === state.traceLimit) {
      state.tracing = false;
    }
  }
  // --- end trace capture ---
...
function evalProg(prog: bril.Program) {
  ...
  // splicing out tracing flag
  let tracing = false;
  const tIdx = args.indexOf("--trace");
  if (tIdx > -1) {
    tracing = true;
    args.splice(tIdx, 1);
  }
  ...
  evalFunc(main, state);
  // if we ever ran in trace mode, write out the JSON and exit
  if (tracing) {
    Deno.writeTextFileSync(
      "trace.json",
      JSON.stringify(state.traceInstrs, null, 2),
    );
    console.error("wrote trace.json with", state.traceCount, "instructions");
    Deno.exit(0);
  }
